...
def download_date_data(code, flag):
    try:
        ...
        stock_zh_a_hist_df.to_csv(os.path.join(os.path.dirname(os.path.abspath(__file__)),f'data_{flag}/{code}.csv'))
...
def download_all_date_data(flag):
    ...
    # 创建保存路径
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)),f'data_{fg}')
...
def get_all_date_data(start_time, end_time, list_assets):
    data_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),'data_bfq')
